...
@property (nonatomic, copy) APPLocalNotificationBlock internalBlock;

@property (strong, nonatomic) UIView * screenBlankView;//empty View

@end

@implementation APPNotificationEvent
...
-(void)presentInView:(UIView *)view withGravityAnimation:(BOOL)animation {
    ...
    if (!_isBeingShown) {
        ...
        _screenBlankView.frame   = CGRectMake(0, CGRectGetHeight(_notificationView.frame), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-CGRectGetHeight(_notificationView.frame));
        _screenBlankView.backgroundColor = [UIColor clearColor];
        [[[UIApplication sharedApplication]keyWindow]addSubview:_screenBlankView];
        ...
        _imageView.frame = CGRectMake(kDropdownPadding, (notificationHeight / 2) - (kDropdownImageSize / 2) - (10 / 2), kDropdownImageSize, kDropdownImageSize);
        ...
        [_screenBlankView addGestureRecognizer:tap];
        if (_dismissOnTap) {
...
-(void)topButtonTapped {
    ...
    if ([self.delegate respondsToSelector:@selector(dropDownNotifiationItemTapped:)])
    {
        [self.delegate dropDownNotifiationItemTapped:nil];
    }
    if (_internalBlock) {
...
-(void)bottomButtonTapped {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(dropDownNotifiationItemTapped:)])
    {
...
-(void)dismissWithGravityAnimation:(BOOL)animation {
    ...
    if (animation) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            ...
            [_screenBlankView removeFromSuperview];
