...
-(APPResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    APPDataBase *self = (__bridge APPDataBase*)f;
...
- (void)resultSetDidClose:(APPResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (DBStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (DBStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(DBStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(DBStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    APPResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (APPResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    DBStatement *statement  = 0x00;
    APPResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[DBStatement alloc] init];
    ...
    rs = [APPResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (APPResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    APPResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[DBStatement alloc] init];
