...
-(instancetype)initWithImage:(UIImage *)image
{
    if(self = [super init])
    {
        ...
        self.radius   = (KScreenWidth -widthEx(60*2))/2.f;
        self.scaleRation =  10;
        lastScale = 1.0;
    }
    return  self;
...
-(void)viewDidLoad
{
    ...
    [self.navigationItem setTitle:@"移动和缩放"];
    self.navigationItem.leftBarButtonItem  = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"Faces_backImg"] style:(UIBarButtonItemStylePlain) target:self action:@selector(dismiss)];
}

-(void)dismiss{
    if ([self.sourceType isEqualToString:@"ZLPhotoPickerViewController"]) {
...
-(void)CreatUI
{
    ...
    [clipBtn setTitleColor:UIColorFromRGB(0xffffff) forState:UIControlStateNormal];
    [clipBtn addTarget:self action:@selector(clipBtnSelected:) forControlEvents:UIControlEventTouchUpInside];
    [clipBtn setBackgroundColor:[UIColorFromRGB(0x000000)colorWithAlphaComponent:0.6]];
    [clipBtn setFrame:CGRectMake(self.view.frame.size.width /2.f, self.view.frame.size.height - heightEx(60), self.view.frame.size.width /2.f, heightEx(60))];
    ...
    [cancel setBackgroundColor:[UIColorFromRGB(0x000000)colorWithAlphaComponent:0.6]];
    [cancel setFrame:CGRectMake(0, self.view.frame.size.height - heightEx(60), self.view.frame.size.width /2.f, heightEx(60))];
    [self.view addSubview:cancel];
    [self drawClipPath:self.clipType];
    [self MakeImageViewFrameAdaptClipFrame];
}

-(void)cancel:(UIButton*)item{
    [self dismiss];
}
...
-(void)drawClipPath:(ClipType )clipType
{
    ...
    UIBezierPath * path= [UIBezierPath bezierPathWithRect:CGRectMake(-2,-2, ScreenWidth+4, ScreenHeight+4)];
