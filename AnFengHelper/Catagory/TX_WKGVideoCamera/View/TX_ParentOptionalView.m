...
@interface TX_ParentOptionalView ()<UIScrollViewDelegate>
...
@implementation TX_ParentOptionalView
...
-(void)scrollToIndex:(NSInteger)index{
    if (index <0 || index > self.containers.count)return;
    [self.containers enumerateObjectsUsingBlock:^(UILabel* _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.alpha = 0.7f;
    }];
    UILabel * touchView = (UILabel*)self.containers[index];
