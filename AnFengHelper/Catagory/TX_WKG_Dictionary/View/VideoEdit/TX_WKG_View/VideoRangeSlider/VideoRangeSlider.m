...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    self.middleLine = ({
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"mline"]];
...
- (void)layoutSubviews {
    ...
    self.middleLine.bounds = CGRectMake(0, 0, 2, self.height);
}


- (void)setAppearanceConfig:(RangeConfig *)appearanceConfig{
...
- (void)setImageList:(NSArray *)images
{
    ...
    CGFloat width =  [self.rangeContent intrinsicContentSize].width;
    self.bgScrollView.contentSize = CGSizeMake(width, CGRectGetHeight(self.frame));
    self.bgScrollView.height = CGRectGetHeight(self.frame);
...
- (void)setCurrentPos:(CGFloat)currentPos
{
    ...
    if (_startColor) {
        ...
        info.colorView.frame = CGRectMake(x, 0, width, self.rangeContent.height - _appearanceConfig.borderHeight*2.f);
    }
    self.disableSeek = NO;
}
